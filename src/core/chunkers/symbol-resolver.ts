...
export type UnmatchedReason =
  ...
  /** A live page elsewhere in this source, in the SAME language, declares that
   *  symbol — under exactly this qualified name, or under this bare name — and
   *  the token is not also a builtin. The definition is real and only the
   *  same-file scope kept us from it. Resolvable by widening the resolver,
   *  which is deliberately out of scope for this pass. */
  ...
  | 'builtin_or_external'
  /** The bare token names BOTH a builtin and a user symbol elsewhere in the
   *  source (`items.filter(…)` in a project that also defines `filter()`).
   *  The extractor emits the same token for both, so neither answer is
   *  defensible. Resolvable only with receiver-type information. */
  | 'ambiguous_bare_token'
...
export async function resolveSymbolEdgesIncremental(
  ...
  while (processed < maxChunks) {
    ...
    // `page_kind = 'code'` bounds the sweep to chunks that can carry symbol
    // edges. Without it, bumping EDGE_EXTRACTOR_VERSION_TS makes every prose and
    // image chunk in the brain stale, and the resolver rewrites the whole corpus
    // 200 rows at a time — on a brain with no code at all. Mirrors the same
    // filter in code-graph-readiness.ts `pendingEdgeChunksExist`.
...
async function processChunkBatch(
  ...
  const needsCrossFileProbe: Array<{ edgeId: number; target: string; language: string | null }> = [];
  /** edge ids grouped by the reason they went unmatched, for one UPDATE per reason. */
  const unmatchedByReason = new Map<UnmatchedReason, number[]>();
  const noteUnmatched = (reason: UnmatchedReason, edgeId: number): void => {
    stats.edges_unmatched += 1;
    stats.unmatched_buckets[reason] += 1;
    const ids = unmatchedByReason.get(reason) ?? [];
    ids.push(edgeId);
    unmatchedByReason.set(reason, ids);
  };
  ...
  for (const e of edges) {
    ...
    if (pageId === undefined) {
      ...
      noteUnmatched('no_candidate_anywhere', e.id);
      continue;
    }
  ...
  if (needsCrossFileProbe.length > 0) {
    // Match the target BOTH ways, and only against declarations in the CALLER'S
    // language. `symbol_name_qualified` is an exact hit; `symbol_name` (the bare
    // name) catches the far more common shape, where the extractor emitted a
    // bare `get` and the definition is stored qualified as `Widget.get`.
    ...
    const rows = await engine.executeRaw<{
      ...
      `SELECT DISTINCT cc.language, cc.symbol_name_qualified, cc.symbol_name
         FROM content_chunks cc
         JOIN pages p ON p.id = cc.page_id
        WHERE p.source_id = $1
    ...
    for (const { edgeId, target, language } of needsCrossFileProbe) {
      ...
      if (declared && builtin) {
        // `items.filter(…)` in a project that also declares `filter()`. The
        // extractor emits the same bare token for both, and a top-level
        // declaration's qualified name IS its bare name — so an exact hit is no
        // stronger here. Claiming either answer would be a guess dressed as a
        // diagnosis.
        noteUnmatched('ambiguous_bare_token', edgeId);
      } else if (declared) {
        noteUnmatched('cross_file_same_source', edgeId);
      } else if (builtin) {
        noteUnmatched('builtin_or_external', edgeId);
      } else {
        noteUnmatched('no_candidate_anywhere', edgeId);
      }
    }
  }
  ...
  // Stamp the reason onto the edge itself, one statement per reason. Without
  // this the buckets live only inside the walk that produced them: the next
  // cycle finds nothing pending, reports all-zero, and the operator never sees
  // the diagnosis. Persisted, they are a property of committed graph state and
  // `symbolUnmatchedBuckets()` can read them back at any time.
...
/**
 * Brain-wide unmatched buckets, read back from committed `edge_metadata`.
 *
 * `ResolverStats.unmatched_buckets` only ever describes the edges THIS walk
 * examined. Once the watermark sweep finishes, every later cycle walks zero
 * chunks and would report all-zero — closing the diagnostic window on the very
 * brains that need it. The resolver therefore stamps each unmatched edge with
 * its reason, and this reads them back.
 *
 * Edges the resolver has never visited carry no `unmatched_reason` and are
 * absent from the result; they are neither explained nor miscounted.
 */
export async function symbolUnmatchedBuckets(
  engine: BrainEngine,
  opts: { sourceId?: string } = {},
): Promise<UnmatchedBuckets> {
  const params: unknown[] = [];
  let scope = '';
  if (opts.sourceId) {
    params.push(opts.sourceId);
    scope = `AND e.source_id = $${params.length}`;
  }
  const rows = await engine.executeRaw<{ reason: string; n: number }>(
    `SELECT e.edge_metadata->>'unmatched_reason' AS reason, count(*)::int AS n
       FROM code_edges_symbol e
       JOIN content_chunks cc ON cc.id = e.from_chunk_id
       JOIN pages p ON p.id = cc.page_id
      WHERE e.edge_metadata ? 'unmatched_reason'
        AND p.deleted_at IS NULL
        ${scope}
      GROUP BY 1`,
    params,
  ...
  for (const row of rows) {
    ...
    // A reason written by a newer gbrain than this one is counted nowhere
    // rather than silently folded into a bucket it doesn't belong to.
    if (reason in buckets) buckets[reason] += Number(row.n);
  }
  return buckets;
}
